...
def main(args):
    ...
    for ep in range(args.max_ep):
        ...
        # env.render()
        ep_rewards = 0
        for step in range(env.spec.timestep_limit):    #env.spec.timestep_limit = 200
            # pick action
...
def args_parse():
    ...
    parser.add_argument(
        '--max_ep', type=int, default=20000, help='Number of training episodes')
    parser.add_argument(
        '--test_ep', type=int, default=50, help='Number of test episodes')
    parser.add_argument(
        '--init_epsilon', type=float, default=0.75, help='initial epsilon')
    parser.add_argument(
        '--final_epsilon', type=float, default=0.05, help='final epsilon')
    parser.add_argument(
